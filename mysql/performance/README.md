...
### 개요
- 실제 현업에서 DB를 사용하다 보면 성능이 느려지는 경우를 반드시 만남

  1. **동시 사용자 수의 증가**: 많은 사용자가 한꺼번에 접속할 때
  2. **데이터 양의 증가**: DB에 쌓인 데이터가 많아질수록 조회 속도 저하
  3. **비효율적인 SQL문 작성**: 잘못 작성된 쿼리는 시스템 자원 낭비

- 데이터베이스 데이터를 사용하는데 처리시간을 줄이기 위한 방안

#### DB 성능 개선 방법의 종류

- DB에 부하가 걸렸을 때 성능을 개선할 수 있는 방법

|방법|설명|
|---|---|
|**SQL 튜닝**|SQL문 자체를 효율적으로 수정하여 성능을 높임|
|**캐싱 서버 활용**|Redis 등을 사용하여 자주 찾는 데이터를 메모리에 저장|
|**레플리케이션**|Master/Slave 구조로 데이터를 복제하여 부하 분산|
...
성능 문제를 해결하기 위한 SQL 튜닝의 핵심은 결국 스토리지 엔진의 부하를 줄이는 데 있음

원칙 1: 스토리지 엔진에서 데이터를 찾기 쉽게 바꾸기

- 스토리지 엔진이 디스크를 뒤지는 시간을 최소화해야
- 데이터를 효율적으로 찾을 수 있는 '길'을 만들어 주는 것

원칙 2: 스토리지 엔진으로부터 가져오는 데이터의 양 줄이기

- 불필요하게 많은 데이터를 엔진 사이에서 주고받지 않도록 쿼리를 최적화해야
- 가져오는 데이터 자체가 적으면 처리 속도는 자연스럽게 속도 개선
